...
rule nanovar_call:
    ...
    output:
        vcfgz=config["dir_data"] + "variants_raw/{cohort}/nanovar/samples/{cohort}.{sample}.{ref_name}.{tech}.nanovar.SV.raw.vcf.gz",
    ...
    log:
        config["dir_data"] + "variants_raw/{cohort}/nanovar/logs/{cohort}.{sample}.{ref_name}.{tech}.nanovar.SV.log",

    benchmark:
        config["dir_data"] + "variants_raw/{cohort}/nanovar/logs/{cohort}.{sample}.{ref_name}.{tech}.nanovar.SV.rtime.tsv",
    ...
    run:
        ...
        nanovar = config["software"]["nanovar"]
        bcftools = config["software"]["bcftools"]
        nanovar_prefix = "/".join(nanovar.split("/")[:-1])
        ref_prefix = "/".join(f"{input.ref}".split("/")[:-1])
        ref_file = f"{input.ref}".split("/")[-1]
        # bgzip = config["software"]["bgzip"]
        shell("date > {log}")
        shell("cd {workdir} && export PATH={nanovar_prefix}:$PATH && "
              "ln -sf {input.ref} ./{ref_file}  && "
              "{nanovar} --data_typ {preset} -t {threads} {input.bam} ./{ref_file} {workdir} "
              "2>>{log} 1>>{log}")
